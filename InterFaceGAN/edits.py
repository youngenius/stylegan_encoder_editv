...
def parse_args():
  ...
  parser.add_argument('-i', '--input_latent_codes_path', type=str, default='/home/ubuntu/gan/pytorch_stylegan_encoder/dlatents',
  ...
  parser.add_argument('--start_distance', type=float, default=-7.0,
                      help='Start point for manipulation in latent space. '
                           '(default: -3.0)')
  parser.add_argument('--end_distance', type=float, default=7.0,
...
def main():
  ...
  latent_codes = []
  paths = glob.glob(os.path.join(args.input_latent_codes_path, '*.npy'))
  for i, path in enumerate(paths):
    latent_code = np.load(path)
    latent_codes.append(model.preprocess(latent_code, **kwargs))

  latent_codes = np.array(latent_codes)
  #np.save(os.path.join(args.output_dir, 'latent_codes.npy'), latent_codes)
  latent_codes = latent_codes.reshape(-1, 18, 512)
